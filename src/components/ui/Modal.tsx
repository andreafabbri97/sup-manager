  ...
  const modalContent = (
    <div className={`fixed inset-0 z-[99999] flex ${containerAlignment} justify-center p-0 sm:p-4 bg-black/50 sm:bg-black/40 backdrop-blur-sm ${isClosing ? 'opacity-0' : 'opacity-100'} transition-opacity duration-200`} style={containerStyle} onClick={requestClose} onPointerDown={requestClose}>
      <div
        ...
        aria-modal="true"
        aria-label={title}
className={`bg-white dark:bg-slate-800 ${dialogRadius} shadow-2xl w-full sm:max-w-[min(640px,70vw)] md:max-w-[min(800px,60vw)] lg:max-w-[min(960px,56vw)] xl:max-w-[min(1100px,50vw)] 2xl:max-w-[min(1280px,45vw)] overflow-hidden transform transition-transform duration-300 ease-out scale-100 sm:animate-modal-open ${isClosing ? 'animate-slide-down' : 'animate-modal-open'} touch-manipulation z-[100000]` }
        onClick={(e) => e.stopPropagation()}
        onPointerDown={(e) => e.stopPropagation()}
      >
