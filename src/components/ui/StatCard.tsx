...
export default function StatCard({ title, value, className = '', color = 'neutral' }: { title: string; value: React.ReactNode; className?: string; color?: 'neutral'|'success'|'warning'|'accent' }) {
  ...
  return (
    <div className={`flex-1 min-w-[12rem] sm:min-w-[13rem] md:min-w-[14rem] max-w-[22rem] p-4 rounded-xl shadow-md min-h-[72px] flex flex-col justify-center transition-transform transform hover:-translate-y-1 ${colorClasses[color]} ${className}`}>
