...
export default function Archive({ start: propStart, end: propEnd }: { start?: string, end?: string } = {}) {
  ...
  function exportCSV() {
    ...
    if (!confirm(`Vuoi esportare ${rows.length} prenotazioni in CSV per il periodo ${start} → ${end}?`)) return
    ...
    a.href = url
    a.download = `bookings-archive-${start}-${end}.csv`
    a.click()
  ...
  function exportInvoicesCSV() {
    ...
    if (rows.length === 0) {
      alert('Nessuna fattura trovata per il filtro')
      return
    }
    if (!confirm(`Vuoi esportare ${rows.length} fatture in CSV per il periodo ${start} → ${end}?`)) return
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `invoices-${start}-${end}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }
    ...
    <section>
      ...
      <div className="overflow-x-auto hidden sm:block">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-neutral-500 dark:text-neutral-300"><th>Data</th><th>Cliente</th><th className="w-36">Prezzo</th><th>Pagato</th><th>Fattura</th></tr>
            </thead>
            <tbody>
              {paginated.map(b => (
